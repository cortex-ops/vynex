...
class ScreenshotProcessor():
    def __init__(self, divisions: Tuple[int, int], output_folder: str):
        self.divisions = divisions
        self.folder = output_folder

    def _calculate_cell_dimensions(self, img_shape: Tuple[int, ...], 
    ...
    def _get_font_properties(self, font_color: Tuple[int, int, int], font_scale: float, 
    ...
    def _get_box_properties(self, box_color: Tuple[int, int, int], 
    ...
    def _draw_label_background_box(self, img: np.ndarray, text_coords: Tuple[int, int],
    ...
    def _draw_label_in_cell(self, img: np.ndarray, label: int, row: int, col: int, 
        ...
        img = self._draw_label_background_box(
    ...
    def add_labels(self, img: np.ndarray, divisions: Tuple[int, int], 
        ...
        cell_dims = self._calculate_cell_dimensions(img.shape, divisions)
        font_props = self._get_font_properties(font_color, font_scale, font_thickness)
        box_props = self._get_box_properties(box_color, box_alpha)
        
        label_number = 1
        for r in range(divisions[0]):
            for c in range(divisions[1]):
                img = self._draw_label_in_cell(
    ...
    def get_labeled_screenshot(self, source: str) -> str:
        ...
        folder_path = make_dir_relative(self.folder)
